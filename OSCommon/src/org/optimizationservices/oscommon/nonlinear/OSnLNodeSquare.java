...
/**
 *
 * <P>The <code>OSnLNodeSquare</code> class represents a square node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeSquare extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "square";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Square function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand of the square function.
	 */
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeSquare(){
		m_mChildren = new OSnLNode[1];
	}//constructor


	/**
	 * Calculate the result value of square function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of square function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.pow(m_mChildren[0].calculateFunction(x), 2);
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * (2 * dValue);
		m_mChildren[0].propogateDerivatives(rootNode, x);
	}//propogateDerivatives

	/**
	 * Calculate the result value of square function derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of square function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		double dDerivative = 2 * m_mChildren[0].calculateFunction(x, functionEvaluated) * m_mChildren[0].calculateDerivative(x, index, functionEvaluated);
		return dDerivative;
	}//calculateDerivative
