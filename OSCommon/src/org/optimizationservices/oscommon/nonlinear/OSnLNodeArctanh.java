/**
 * @(#)OSnLNodeArctanh 1.0 03/14/2004
 *
 * Copyright (c) 2004
 */
...
/**
 *
 * <P>The <code>OSnLNodeArctanh</code> class represents a arctanh node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeArctanh extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "arctanh";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Arctanh function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand of the arctanh function.
	 */
	protected OSnLNodeArctanh(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeArctanh(){
		m_mChildren = new OSnLNode[1];
	}//constructor


	/**
	 * Calculate the result value of arctanh function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of arctanh function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = 0.5 * Math.log((1 + m_mChildren[0].calculateFunction(x)) / (1 - m_mChildren[0].calculateFunction(x)));
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		if(dValue <= -1 || dValue >= 1) m_mChildren[0].m_dNodeDerivative = Double.NaN;
		else m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * (1 / (1 - Math.pow(dValue, 2)));
		m_mChildren[0].propogateDerivatives(rootNode, x);
	}//propogateDerivatives

	/**
	 * Calculate the result value of arctanh function derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of arctanh function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		...
		if(dValue <= -1 || dValue >=1) dDerivative = Double.NaN;
		else dDerivative = m_mChildren[0].calculateDerivative(x, index, functionEvaluated) /
		(1 - Math.pow(dValue, 2));
		return dDerivative;
	}//calculateDerivative
