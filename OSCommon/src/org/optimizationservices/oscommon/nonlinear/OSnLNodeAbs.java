...
/**
 *
 * <P>The <code>OSnLNodeAbs</code> class represents a absolute value node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeAbs extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "abs";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Absolute value function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand to take the absolute value.
	 */
	protected OSnLNodeAbs(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeAbs(){
		m_mChildren = new OSnLNode[1];
	}//constructor

	/**
	 * Calculate the result value of absolute value function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of absolute value function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.abs(m_mChildren[0].calculateFunction(x));
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		if(dValue < 0) m_mChildren[0].m_dNodeDerivative = -m_dNodeDerivative;
		if(dValue > 0) m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative;
	...
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		...
		if(dValue < 0) dDerivative = -m_mChildren[0].calculateDerivative(x, index, functionEvaluated);
		if(dValue > 0) dDerivative = m_mChildren[0].calculateDerivative(x, index, functionEvaluated);
		return dDerivative;
	}//calculateDerivative
