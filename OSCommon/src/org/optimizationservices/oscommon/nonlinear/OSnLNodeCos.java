...
/**
 *
 * <P>The <code>OSnLNodeCos</code> class represents a cos node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeCos extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "cos";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Cos function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand of the cos function.
	 */
	protected OSnLNodeCos(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeCos(){
		m_mChildren = new OSnLNode[1];
	}//constructor


	/**
	 * Calculate the result value of cos function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of cos function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.cos(m_mChildren[0].calculateFunction(x));
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * (-Math.sin(dValue));
		m_mChildren[0].propogateDerivatives(rootNode, x);
	}//propogateDerivatives

	/**
	 * Calculate the result value of cos function derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of cos function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		double dDerivative = -Math.sin(m_mChildren[0].calculateFunction(x, functionEvaluated)) * m_mChildren[0].calculateDerivative(x, index, functionEvaluated);
		return dDerivative;
	}//calculateDerivative
