...
/**
 *
 * <P>The <code>OSnLNodeExp</code> class represents a exponential node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeExp extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "exp";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Exponential function of a node operand";
	 ...
	 */
	protected OSnLNodeExp(OSnLNode node){
	...
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.exp(m_mChildren[0].calculateFunction(x));
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * Math.exp(dValue);
	...
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		...
		dDerivative = m_mChildren[0].calculateDerivative(x, index, functionEvaluated) * Math.exp(dValue);
		return dDerivative;
	}//calculateDerivative
