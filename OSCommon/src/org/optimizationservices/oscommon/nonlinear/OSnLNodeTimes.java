...
/**
 *
 * <P>The <code>OSnLNodeTimes</code> class represents a multiplication node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeTimes extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "times";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Multiplication of 2 node operands";
	 ...
	 * </p>
	 *
	 * @param leftNode holds the left node operand to multiply.
	 * @param rightNode holds the right node operand to multiply.
	 */
	protected OSnLNodeTimes(OSnLNode leftNode, OSnLNode rightNode){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeTimes(){
		m_mChildren = new OSnLNode[2];
	}//constructor

	/**
	 * Calculate the result value of multiplication given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of multiplication given the current variable values.
	 */
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * (dRightValue);
		m_mChildren[1].m_dNodeDerivative = m_dNodeDerivative * (dLeftValue);
		m_mChildren[0].propogateDerivatives(rootNode, x);
		m_mChildren[1].propogateDerivatives(rootNode, x);
	}//propogateDerivatives

	/**
	 * Calculate the result value of multiplication derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of multiplication derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		double dDerivative = m_mChildren[0].calculateDerivative(x, index, functionEvaluated) * m_mChildren[1].calculateFunction(x, functionEvaluated)
