...
/**
 *
 * <P>The <code>OSnLNodeLn</code> class represents a natural log node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeLn extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "ln";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Natural log function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand to take the natural log function.
	 */
	protected OSnLNodeLn(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeLn(){
		m_mChildren = new OSnLNode[1];
	}//constructor


	/**
	 * Calculate the result value of natural log function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of natural log function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.log(m_mChildren[0].calculateFunction(x));
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		if(dValue <= 0)  m_mChildren[0].m_dNodeDerivative = Double.NaN;
		else m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * (1 / dValue);
		m_mChildren[0].propogateDerivatives(rootNode, x);
	}//propogateDerivatives

	/**
	 * Calculate the result value of natural log function derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of natural log function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		...
		if(dValue <= 0) dDerivative = Double.NaN;
		else dDerivative = m_mChildren[0].calculateDerivative(x, index, functionEvaluated) / dValue;
		return dDerivative;
	}//calculateDerivative
