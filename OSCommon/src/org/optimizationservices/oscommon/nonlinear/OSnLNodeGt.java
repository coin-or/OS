...
/**
 *
 * <P>The <code>OSnLNodeGt</code> class represents a gt (or greater than) relational node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeGt extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "gt";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Gt (or greater than) relational node";
	 ...
	 * @param rightNode
	 */
	protected OSnLNodeGt(OSnLNode leftNode, OSnLNode rightNode){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeGt(){
		m_mChildren = new OSnLNode[2];
	}//constructor


	/**
	 * Calculate the result value of gt (or greater than) relational operator given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of gt (or greater than) relational operator given the current variable values.
	 * If the left node's calculated value is greater than right node's calculated value, return TRUE.
	 * Else, return FALSE.
	 */
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		if(dLeftValue ==  dRightValue){
			m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * Double.POSITIVE_INFINITY;
			m_mChildren[1].m_dNodeDerivative = m_dNodeDerivative * Double.NEGATIVE_INFINITY;
		}
		else{
	...
	/**
	 * Calculate the result value of gt (or greater than) relational operator derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of gt (or greater than) relational operator derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		...
		if(m_mChildren[0].calculateFunction(x, functionEvaluated) ==  m_mChildren[1].calculateFunction(x, functionEvaluated)){
			...
			else if(dV0 == 0) dDerivative = Double.NEGATIVE_INFINITY * dV1;
			else if(dV1 == 0) dDerivative = Double.POSITIVE_INFINITY * dV0;
			else dDerivative = Double.POSITIVE_INFINITY * dV0 + Double.NEGATIVE_INFINITY * dV1;
		}
		else{
