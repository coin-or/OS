...
\vskip 8pt
\noindent {\bf Notes:}\label{itemize:unixBuildNotes}
...
\ifdevelop
\item The option {\tt --enable-debug} is used to build a debug version of the code.
\item The option {\tt --with-flex-bison} can be used to control whether the parser files should be rebuilt. This requires  {\tt flex} and {\tt bison} (see Section~\ref{section:flex}) and has caused problems on some systems, so the default is
not to use the parsers. Only parser developers should need this option, since the {\tt flex} and {\tt bison} output is part of the standard distribution.
\fi
...
We have also successfully used the {\tt fink} project, see
...
:/cygdrive/c/Program Files/Microsoft SQL Server/90/Tools/bin/
...
\ifbible
\subdivision{CSDP}\label{section:csdp}

CSDP is a package to solve semidefinite programming problems. 
It is available as a COIN-OR project (\url{https://projects.coin-or.org/Csdp}) 
and can be called from OS. However, it does not have an integrated build system and is currently
not part of CoinAll. In order to use CSDP with OS, it is necessary to either download 
a binary distribution or to check out the source with svn and use the included makefile to build the package locally.

Subsequently OS must be configured so that it can find the required library and header files, for instance using

\begin{verbatim}
configure --with-csdp-lib="-L$(HOME)/Csdp/lib -lsdp"        \
          --with-csdp-incdir="$HOME/Csdp/include
\end{verbatim}

Once the OptimizationServices has been built successfully, the CSDP solver can be invoked as follows to solve 
the sample problem {\tt SDPA\_ex.osil}:

{\tt ./OSSolverService solver csdp osil \$DATA/osilFiles/SDPA\_ex.osil}

{\tt SDPA\_ex.osil} is included in the OS distribution. However, at present OS uses a stand-alone converter
for other semidefnite programming problems in the SDPA format. The converter is written in Fortran 
and is found at

{\tt OS/misc/ConeProgramming/SDPA2OSiL.f90}

It is a completely stand-alone program and can be compiled with any Fortran90 compiler. 
It reads an SDPA file from standard input and writes an osil file to standard output. 
...
and see to it that the corresponding
\begin{verbatim}
COIN_HAS_XXXXX
\end{verbatim}
variables are defined in  {\tt config\_os.h}.
\index{COIN_HAS_XXXXX@{\tt COIN\_HAS\_XXXXX}|)}
...
\begin{verbatim}
configure --with-cplex-incdir="$(HOME)/Cplex/cplex/include"  \
--with-cplex-lib="-L$(HOME)/Cplex/cplex/lib -lcplex -lilocplex -lm -lpthread"
...
\subsubdivision{Other solvers}

It is possible to connect other solvers. This requires appropriate {\tt COIN\_HAS\_XXXX} variables to be set, which is accomplished by adding reference to the solver in the {\tt configure.ac} file by calling one of  
...
\noindent
depending on whether the solver has a build system under {\tt COIN\_OR} (the first case) or does not. The file {\tt BuildTools/coin.m4} contains some more information about the use of these two commands. Once the changes to {\tt configure.ac} have been made, it is necessary to run the autotools again, followed by a {\tt configure} step (making sure to include appropriate {\tt lib} and {\tt include} directories) as well as a {\tt make install}. 
...
\vskip 8pt
 ...
 Linux &Ubuntu  7.10 &gcc 4.1.2& Dell Intel 32 or 64 bit chip\\ \hline
 Cyg-gcc &Windows 2003 Server&gcc 4.2.2& Dell Intel 32 or 64 bit chip \\ \hline
 Msys-cl &Windows XP&cl 14.00 &Dell Intel 32 or 64 bit chip \\ \hline
 MinGW-gcc &Windows XP&gcc 3.4.2&Dell Intel 32 or 64 bit chip \\ \hline
 MSVS &Windows XP&Visual Studio 9 and 10 &Dell Intel 32 or 64 bit chip \\ \hline
\end{tabular}
\end{table}
