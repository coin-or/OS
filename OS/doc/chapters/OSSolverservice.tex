...
this is used as input  to the {\tt knock} method. If {\tt serviceMethod knock} is specified, this parameter is also needed.

\item[] {\tt osplOutput xxx.ospl}\ \  The name of an output file in the  OS Process Language (OSpL); 
this is the  output  string from the {\tt knock}  and {\tt kill} method. If not present, the ouput is displayed to the terminal screen.
...
\item[] {\tt solver  solverName}\ \ {\bf Note that this option only has effect for local calls.} 
...
\item[] {\tt logFile xxx}\ \ This parameter specifies a secondary output deviceto which output can be directed in addition to {\tt stdout}. 

\item[] {\tt filePrintLevel nnn}\ \ This parametercontrols the amount of output sent to the secondary output device selected by {tt logFile}.
...
A string is returned with the {\tt JobID}. This {\tt JobID} can then be put into a {\tt <jobID>} element in an
