...
using XSLT into HTML and displayed in the browser. This parameter can only be used in conjunction with the {\tt osrl} parameter. In addition, some browsers require that the stylesheet OSrL.xslt is found in the same directory as the result file.
...
One such solver service is maintained at
\begin{verbatim}
...
\ifdevelop
\begin{verbatim}
{\tt osol $DATA/data/osolFiles/remoteSolve1.osol}
\end{verbatim}
\else
\begin{verbatim}
{\tt osol ../examples/data/osolFiles/remoteSolve1.osol}
\end{verbatim}
\fi
...
<osol xmlns="os.optimizationservices.org"
    ...
    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="os.optimizationservices.org http://www.optimizationservices.org/schemas/2.0/OSoL.xsd">
    <general>
...
%So the {\tt OSSolverService} is running on the server {\tt http://74.94.100.129:8080/} 
%which contacts the server {\tt www.coin-or.org} for the model instance.
...
The {\tt <contact>} element can be used to trigger an email message once the job has finished. Another way to determine the status of a job uses th {\tt knock} method (see Section~\ref{section:knock}.)
