...
(Make sure to replace `{\tt xxx.xxx.xxx.xxx}' by the IP address of Kipp Martin's server.)
...
Before preparing the release, edit the {\tt Dependencies} file to point to the latest available stable versions for all the external packages and edit {\tt configure.ac} to store the correct version
(see Section~\ref{section:VersionNumbering}.)
...
{\tt prepare\_new\_stable} uses svn to check out the code and should be instructed to locate the latest stable versions of all the dependencies, even if the basis uses trunk versions.
...
\noindent
where {\tt dir} is the local directory that contains the new stable version, e.g., {\tt ./OS-2.10}. In this form, it only performs a ``dry run'', that is, it only prints the commands that are to be executed, without actually doing anything. Once satisfied, use the command line parameter {\tt -c} to do the commit.



{\tt prepare\_new\_release} and {\tt commit\_new\_release} are the corresponding scripts to prepare and commit releases (which are based on stable versions). The scripts determine release numbers automatically. For instance, a new maintenance release of {\tt stable\_2.9} to follow release 2.9.3 would use

\begin{verbatim}
prepare_new_release OS/stable/2.9
\end{verbatim}

and would automatically increase the release number to 2.9.4. If this script finishes without error, simply say

\begin{verbatim}
commit_new_release 
\end{verbatim}

to create the folder {\tt OS/releases/2.9.4} in the code repository.
...
\begin{verbatim}
svn merge https://projects.coin-or.org/svn/OS/stable/2.10      \
          https://projects.coin-or.org/svn/OS/trunk@5047    .
\end{verbatim} 
