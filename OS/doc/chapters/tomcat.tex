...
\item If the Tomcat server is working, submit a job to the remote OSSolverService with the following
{\tt serviceLocation}:
...
This tests the connection with the OSSolver Java servlet.

\item Finally, try call the OSSolverService.exe in the normal way. If this fails, the remote OSSolverService is not running properly. At this point you may also want to check whether the local solver gives sensible output when run on the same data.
