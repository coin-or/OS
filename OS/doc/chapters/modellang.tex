...
\chaptermark{OS Support for other environments and formats}
\else
...
\end{verbatim}
where {\tt xxx.xxx.xxx.xxx} is the IP Address for the server. This string is used to replace the string `{\tt solver symphony}' in the previous example. 
The {\tt serviceLocation} option will send the problem to the %solver server at 
location {\tt http://xxx.xxx.xxx.xxx} and, assuming the remote executable is indeed found 
...
installation dependent.
%the solver service at 
%{\tt http://xxx.xxx.xxx.xxx:8080/OSServer/services/OSSolverService} accepts
%{\tt clp},  
