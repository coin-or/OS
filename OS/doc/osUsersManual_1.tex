...
x^{(k)} = \frac{1}{k!}X^{(k)}(0)
\end{eqnarray*}
Then $Y(t) = f(X(t))$ is a function from $ \mathbb{R}^{n}$ to $ \mathbb{R}^{m}$ is expressed in terms of its Taylor series expansion as
\begin{eqnarray}
Y(t)  = y^{(0)} +  y^{(1)} t +  y^{(2)} t^{2} + o(t^{3})
\end{eqnarray} 
where 

kippster put in taylor coefficient

\subsection{Using OSInstance Methods}
...
\end{verbatim}
where $p$ is the highest order Taylor coefficient of $f$ to be calculated,  $\tt vdX$ is vector of doubles in $ \mathbb{R}^{n},$ and the function return is a vector of doubles in $ \mathbb{R}^{m}.$  For example, the following call will evaluate each component function defined in \ref{eq:definef}.
\begin{verbatim}
...
\end{verbatim}
Since there are three components in the vector defined by  \ref{eq:definef}, the return value  {\tt funVals} will have three components. For the given input vector, the values of these components are 1, -63.15, and 1.6094.
