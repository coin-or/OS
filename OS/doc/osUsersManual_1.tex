...
\subsubsection{Converting AMPL nl Files}
...
The OS library provides for algorithmic differentiation.  For an excellent reference on algorithmic differentiation see Griewank \cite{griewank2000}.  The OS package uses the COIN-OR package CppAD which has excellent documentation and information about algorithmic differentiation. See the documentation from Brad Bell at  \url{http://www.coin-or.org/CppAD/}.  

First we provide some algorithmic  differentiation basics. 

{\bf Assumptions:} 

\begin{itemize}

\item[1.] The solver will make use of function callbacks for function evaluations (objective function and constraints), gradient evaluations (objective function and Jacobian of the constraints), and Hessian calculation (usually the Hessian of the Lagrangian function).

\item[2.] The first and second order derivatives will be evaluated by an AD (Automatic Differentiation) package such as CppAD.

\end{itemize}

Consider the function $f:X \rightarrow Y$ from $ \mathbb{R}^{n}$ to $ \mathbb{R}^{m}.$
Algorithmic differentiation is done using either a forward or reverse sweep through an expression tree. Again, see Griewank book or Bell  online documentation.  The two key methods in the {\tt OSInstance} class are {\tt forwardAD} and {\tt reverseAD}.   The {\tt forwardAD} signature is
\begin{verbatim}
std::vector<double> forwardAD(int p, std::vector<double> vdX);
\end{verbatim}
where $p$ is the highest order Taylor coefficient calculated and $\tt vdX$ is vector of doubles in $ \mathbb{R}^{n}$ and the function return is a vector of doubles in $ \mathbb{R}^{m}.$  Consider the following example.

\begin{alignat}{2}
& \mbox{Minimize} & \quad  x_{0}^{2} + 9x_{1} \label{eq:adobj}\\
& \mbox{Subject to} & 33 - 105 + 1.37 x_{1} + 2x_{2} + 5 x_{1} &\le 10  \label{eq:adeq0}\\
& & \ln(x_{0} x_{2}) + 7x_{3} &\ge 10 \label{eq:adeq1} \\
& & x_{0}, x_{1}, x_{2}, x_{3} &\ge 0 \label{eq:adeq2}
\end{alignat}

The OSiL representation of the instance  (\ref{eq:adobj})-(\ref{eq:adcon2}) is given in Appendix \ref{section:adexample}.  This example is designed to illustrate several features of OSiL. Note that in equation (\ref{eq:adeq0}) the constant 33 appears in the {\tt <con>} element corresponding to this constraint and the constant 105 appears as a {\tt <number>} OSnL node in the {\tt <nonlinearExpressions>} section. Although variable $x_{1}$ does not appear in any nonlinear expression the $5 x_{1}$ term in equation (\ref{eq:adeq0}) is expressed in the {\tt <linearConstraintCoefficients>} section and the $1.37 x_{1}$ term in equation (\ref{eq:adeq0}) is expressed in the {\tt <nonlinearExpressions>} section.  Hence, in the OSInstance API, variable $x_{1}$ is treated as a nonlinear variable for purposes of algorithmic differentiation. However, variable $x_{3}$ never appears in the  {\tt <nonlinearExpressions>} section and is therefore treated as a linear variable and never appears in algorithmic differentiation calculations. 

Ignoring the nonnegativity constraints, instance (\ref{eq:adobj})-(\ref{eq:adcon2})  defines the following function $f:X \rightarrow Y$ from $ \mathbb{R}^{4}$ to $ \mathbb{R}^{3}.$
...
Kipp -- important -- point out that linear variables never are part of the AD


Kipp -- important  -- point out that we list constant terms in the Jacobian
...
%%%%%%%%%%%%%%%
%%% Below is email exchange  with Andreas
%%%%%%%%%%%%%%%

{\bf Key Idea:} as part of the callback function signatures (arguments) the solver should pass (or make available another way) the following {\it two variables}:

\begin{itemize} 

\item[]   {\bf new\_x} this is boolean and defined as in Ipopt. It is false if any evaluation method was previously called with the same {\bf x}, true otherwise

\item[]  {\bf highestOrder} this is an integer variable that will take on the value 0, 1, or 2 (actually higher values if we want third derivatives etc.).  The value of this variable is the highest order derivative that will be required of the current iterate. For example, if  a callback required a function evaluation and {\bf highestOrder = 0} then only the function need be evaluated at the current iterate.  If, however, {\bf highsetOrder = 2} then we would make a first and second derivative calculation in addition to the function evaluation.

\end{itemize}


In order to implement AD efficiently  the code that implements the callback functions (such as our  OS library API) would store a class variable {\bf highestOrderEvaluated}. Every time a forward or reverse sweep is executed this variable gets set to the order of the sweep. For example, {\tt f.forward(1, x)} would result in {\bf highestOrderEvaluated = 1}.  Just knowing the value  of {\bf new\_x} alone is not sufficient. It is also necessary for the AD implementation to know {\bf highestOrder} and {\bf highestOrderEvaluated.}  For example, {\bf new\_x} might be false because of the calculation {\tt f.forward(0, x)}, however if  the callback requires a Hessian calculation, then it is necessary to calculate the first and second derivative at the current iterate. 

Given this information there are {\it  exactly two} conditions that  require a new function or derivative calculation.   A new evaluation is required if and only if
...
\item[2.] For the callback function the value of the input parameter {\bf highestOrder} is strictly greater than the current value  of    {\bf highestOrderEvaluated.} 
\end{itemize}



It would be nice if all of the function callbacks in the solver API had the following arguments:

\begin{itemize}

\item[] {\bf double *x} a pointer to the current iterate primal values

\item[] {\bf double *objLambda} a pointer to the objective function (there may be more than 1) Lagrange multipliers

\item[] {\bf double  *conLambda} a pointer to the constraint Lagrange multipliers

\item[] {\bf int objIdx} the index of the objective function of interest -- this is optional.

\item[] {\bf new\_x} explained above

\item[]  {\bf highestOrder} explained above
...
To make a long story short -- from the standpoint of implementing AD it would be nice to know if when the solver is calling a function evaluation it is doing so only for a line search, or if a Hessian will also be required. 
