/* $Id: OSnl2osil.cpp 4502 2012-05-31 10:30:02Z Gassmann $ */
/** @file OSnl2osil.cpp
 *
...
asl=rw;
    for(i = 0; i < n_con; i++)
    {
std::cout << "Cgrad[i]: " << Cgrad[i] << std::endl;
        for(cg = Cgrad[i]; cg; cg = cg->next)
        {
            if (fabs(cg->coef) > OS_EPS)
            {
std::cout << "con coeff in obj row " << i << " col " << cg->varno << ": " << cg->coef << std::endl;
            }
        }
    }
asl=cw;
