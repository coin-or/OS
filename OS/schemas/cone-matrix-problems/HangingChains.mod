...
minimize pot_energy: sum {j in 1..N-1} m[j]*g*y[j] 
	     + (k/2)*sum {j in 1..N}   t[j]^2 ;
...
subject to link_up {j in 1..N}:     # the cone constraints
    sqrt((x[j] - x[j-1])^2 + (y[j] - y[j-1])^2)  <= l0 + t[j];

# Use transformation u[j] = x[j] - x[j-1], v[j] = y[j] - y[j-1].
