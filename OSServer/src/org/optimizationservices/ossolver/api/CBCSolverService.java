/**
 * @(#)CBCSolverService 1.0 03/14/2004
 *
 * Copyright (c) 2004
 */
...
/**
 *
 * <P>The <code>CBCSolverService</code> is the CBC Solver's Web Service facade. 
 *
 * </p>
 ...
 * @since OS 1.0
 */
public class CBCSolverService implements OShL{
	/**
	...
	public CBCSolverService(){
		...
		try {
			String sClassName = OSParameter.SOLVER_CLASS_NAME; //"org.optimizationservices.ossolver.solver.CBCSolver";
			Class solverClass = Class.forName(sClassName);
			m_osServiceUtil.solver =(DefaultSolver)solverClass.newInstance();
			//m_osServiceUtil.solver = new CBCSolver();
		} 
		catch (Exception e) {
	 ...
	 */
	static{
		//OSParameterFile.NAME = IOUtil.getCurrentDir()+ "webapps/cbc/code/OSConfig/OSParameter_CBC.xml";
		OSParameterFile.NAME = "/home/kmartin/bin/tomcat/webapps/cbc/code/OSConfig/OSParameter_CBC.xml";
		//OSParameterFile.NAME = "/code/OSConfig/OSParameter_CBC.xml"; 

	}//static constructor
}//class CBCSolverService
