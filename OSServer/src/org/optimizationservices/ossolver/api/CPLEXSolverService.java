/**
 * @(#)CPLEXSolverService 1.0 03/14/2004
 *
 * Copyright (c) 2004
 */
...
/**
 *
 * <P>The <code>CPLEXSolverService</code> is the CPLEX Solver's Web Service facade. 
 *
 * </p>
 ...
 * @since OS 1.0
 */
public class CPLEXSolverService implements OShL{
	/**
	...
	public CPLEXSolverService(){
		...
		try {
			String sClassName = OSParameter.SOLVER_CLASS_NAME; //"org.optimizationservices.ossolver.solver.CPLEXSolver";
			Class solverClass = Class.forName(sClassName);
			m_osServiceUtil.solver =(DefaultSolver)solverClass.newInstance();
			//m_osServiceUtil.solver = new CPLEXSolver();
		} 
		catch (Exception e) {
	 ...
	 */
	static{
		//OSParameterFile.NAME = IOUtil.getCurrentDir()+ "webapps/cplex/code/OSConfig/OSParameter_CPLEX.xml";
		OSParameterFile.NAME = "/home/kmartin/bin/tomcat/webapps/cplex/code/OSConfig/OSParameter_CPLEX.xml";
		//OSParameterFile.NAME = "/code/OSConfig/OSParameter_CPLEX.xml"; 

	}//static constructor
}//class CPLEXSolverService
