/**
 * @(#)IPOPTSolverService 1.0 03/14/2004
 *
 * Copyright (c) 2004
 */
...
/**
 *
 * <P>The <code>IPOPTSolverService</code> is the IPOPT Solver's Web Service facade. 
 *
 * </p>
 ...
 * @since OS 1.0
 */
public class IPOPTSolverService implements OShL{
	/**
	...
	public IPOPTSolverService(){
		...
		try {
			String sClassName = OSParameter.SOLVER_CLASS_NAME; //"org.optimizationservices.ossolver.solver.IPOPTSolver";
			Class solverClass = Class.forName(sClassName);
			m_osServiceUtil.solver =(DefaultSolver)solverClass.newInstance();
			//m_osServiceUtil.solver = new IPOPTSolver();
		} 
		catch (Exception e) {
	 ...
	 */
	static{
		//OSParameterFile.NAME = IOUtil.getCurrentDir()+ "webapps/ipopt/code/OSConfig/OSParameter_IPOPT.xml";
		OSParameterFile.NAME = "/home/kmartin/bin/tomcat/webapps/ipopt/code/OSConfig/OSParameter_IPOPT.xml";
		//OSParameterFile.NAME = "/code/OSConfig/OSParameter_IPOPT.xml"; 

	}//static constructor
}//class IPOPTSolverService
