/**
 * @(#)OSnLNodeSquareRoot 1.0 03/14/2004
 *
 * Copyright (c) 2004
 */
...
/**
 *
 * <P>The <code>OSnLNodeSquareRoot</code> class represents a square root node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeSquareRoot extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "squareRoot";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Square root function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand of the square root function.
	 */
	protected OSnLNodeSquareRoot(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeSquareRoot(){
		m_mChildren = new OSnLNode[1];
	}//constructor

	/**
	 * Calculate the result value of square root function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of square root function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.sqrt(m_mChildren[0].calculateFunction(x));
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * (0.5 / Math.sqrt(dValue));
		m_mChildren[0].propogateDerivatives(rootNode, x);
	}//propogateDerivatives

	/**
	 * Calculate the result value of square root function derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of square root function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		double dDerivative = 0.5 * m_mChildren[0].calculateDerivative(x, index, functionEvaluated) / Math.sqrt(m_mChildren[0].calculateFunction(x, functionEvaluated));
		return dDerivative;
	}//calculateDerivative
