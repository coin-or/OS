...
/**
 *
 * <P>The <code>OSnLNodeArccos</code> class represents a arccos node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeArccos extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "arccos";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Arccos function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand of the arccos function.
	 */
	protected OSnLNodeArccos(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeArccos(){
		m_mChildren = new OSnLNode[1];
	}//constructor

	/**
	 * Calculate the result value of arccos function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of arccos function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.acos(m_mChildren[0].calculateFunction(x));
	...
	/**
	 * Calculate the result value of arccos function derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of arccos function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		...
		Math.sqrt(1 - Math.pow(m_mChildren[0].calculateFunction(x, true), 2));
		return dDerivative;
	}//calculateDerivative
