...
/**
 *
 * <P>The <code>OSnLNodeCosh</code> class represents a cosh node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeCosh extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "cosh";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Cosh function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand of the cosh function.
	 */
	protected OSnLNodeCosh(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeCosh(){
		m_mChildren = new OSnLNode[1];
	}//constructor


	/**
	 * Calculate the result value of cosh function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of cosh function given the current variable values.
	 */
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		double dSinhValue = (Math.exp(dValue) - Math.exp(-dValue)) / 2;
		m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * (dSinhValue);
		m_mChildren[0].propogateDerivatives(rootNode, x);
	}//propogateDerivatives

	/**
	 * Calculate the result value of cosh function derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of cosh function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		double dSinhValue = (Math.exp(m_mChildren[0].calculateFunction(x, functionEvaluated)) - Math.exp(-m_mChildren[0].calculateFunction(x, functionEvaluated))) / 2;
		double dDerivative =  dSinhValue * m_mChildren[0].calculateDerivative(x, index, functionEvaluated);
		return dDerivative;
	}//calculateDerivative
