...
/**
 *
 * <P>The <code>OSnLNodeFloor</code> class represents a floor value node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeFloor extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "floor";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Floor value function of a node operand";
	...
	/**
	 *
	 * @param node holds the node operand to take the floor function.
	 */
	protected OSnLNodeFloor(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeFloor(){
		m_mChildren = new OSnLNode[1];
	}//constructor


	/**
	 * Calculate the result value of floor value function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of floor value function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.floor(m_mChildren[0].calculateFunction(x));
	...
	/**
	 * Calculate the result value of floor value function derivative given the current variable values.
	 ...
	 * @return the result value of floor value function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		...
		if(Math.abs(dValue - (int)(dValue + 0.5)) <= OSnLNode.m_dIntTol){
			...
			else dDerivative = Double.NEGATIVE_INFINITY * dV;
		}
		return dDerivative;
	}//calculateDerivative
