/**
 * @(#)OSnLNodeCeiling 1.0 03/14/2004
 *
 * Copyright (c) 2004
 */
...
/**
 *
 * <P>The <code>OSnLNodeCeiling</code> class represents a ceiling value node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeCeiling extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "ceiling";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Ceiling value function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand to take the ceiling function.
	 */
	protected OSnLNodeCeiling(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeCeiling(){
		m_mChildren = new OSnLNode[1];
	}//constructor


	/**
	 * Calculate the result value of ceiling value function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of ceiling value function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.ceil(m_mChildren[0].calculateFunction(x));
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		if(Math.abs(dValue - (int)(dValue + 0.5)) <= OSnLNode.m_dIntTol){
	...
	/**
	 * Calculate the result value of ceiling value function derivative given the current variable values.
	 * If the node's calculated value is an integer, return NAN.
	 * Else return 0;
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of ceiling value function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		...
		if(Math.abs(dValue - (int)(dValue + 0.5)) <= OSnLNode.m_dIntTol){
			double dV = m_mChildren[0].calculateDerivative(x, index, functionEvaluated);
			if(dV == 0) dDerivative = 0;
			else dDerivative = Double.POSITIVE_INFINITY * dV;
		}
		return dDerivative;
	}//calculateDerivative
