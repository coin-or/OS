...
/**
 *
 * <P>The <code>OSnLNodeTan</code> class represents a tan node in an expression tree.
 ...
 * @since OS 1.0
 */
public class OSnLNodeTan extends OSnLNode{

	/**
	 * m_sSymbol holds the symbol that this nlNode looks in ASCII.
	 */
	protected String m_sSymbol = "tan";

	/**
	 * m_sSymbol holds the description of this nlNode
	 */
	protected String m_sDescription = "Tan function of a node operand";
	 ...
	 * </p>
	 *
	 * @param node holds the node operand of the tan function.
	 */
	protected OSnLNodeTan(OSnLNode node){
	 ...
	 * Default Constructor
	 */
	protected OSnLNodeTan(){
		m_mChildren = new OSnLNode[1];
	}//constructor

	/**
	 * Calculate the result value of tan function given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @return the result value of tan function given the current variable values.
	 */
	protected double calculateFunction(double[] x){
		m_dFunctionValue = Math.tan(m_mChildren[0].calculateFunction(x));
	...
	protected void propogateDerivatives(OSnLNode rootNode, double[] x){
		...
		m_mChildren[0].m_dNodeDerivative = m_dNodeDerivative * (1 + Math.pow(Math.tan(dValue), 2));
		m_mChildren[0].propogateDerivatives(rootNode, x);
	}//propogateDerivatives

	/**
	 * Calculate the result value of tan function derivative given the current variable values.
	 *
	 * </p>
	 *
	 * @param x holds the values of the variables in a double array.
	 * @param functionEvaluated holds whether the function has been evaluated.
	 * @return the result value of tan function derivative given the current variable values.
	 */
	protected double calculateDerivative(double[] x, int index, boolean functionEvaluated){
		double dDerivative = 1 + Math.pow(Math.tan(m_mChildren[0].calculateFunction(x, functionEvaluated)), 2) * m_mChildren[0].calculateDerivative(x, index, functionEvaluated);
		return dDerivative;
	}//calculateDerivative
